...
recs_core = 
  ...
  transmute( 
    ...
    # grouping factor
    region = factor(REGIONC,labels=c('Northeast', 'Midwest', 'South', 'West'), levels=1:4),
...
temps_by_region1 = recs_core %>% 
  group_by(region) %>%
  dplyr::summarise(
    across(starts_with("temp"), function(x) sum(x * weight) / sum(weight), .names="avg_{.col}")
  )
...
recs_mean0 = function(df, vars) {
  ...
  return(df %>% group_by(region) 
    %>% dplyr::summarise(across(all_of(vars), function(x) sum(x * weight) / sum(weight) )))
}
